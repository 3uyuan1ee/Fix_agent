...
class InputParser:
    ...
    def parse(self, user_input: str) -> ParsedInput:
        ...
        if self._is_exit_command(input_clean):
            return ParsedInput(
                original_input=user_input, input_type=InputType.EXIT, command="exit"
            )

        # 检查帮助命令
        if self._is_help_command(input_clean):
            return ParsedInput(
                original_input=user_input, input_type=InputType.HELP, command="help"
            )
    ...
    def _is_exit_command(self, input_str: str) -> bool:
        """检查是否为退出命令"""
        exit_patterns = [r"^(exit|quit|bye|再见)$", r"^/(exit|quit|stop)$"]
    ...
    def _is_help_command(self, input_str: str) -> bool:
        """检查是否为帮助命令"""
        help_patterns = [r"^(/help|help|\?|？)$", r"^/help\s+.+$"]
    ...
    def _detect_mode_hints(self, text: str) -> List[str]:
        ...
        if any(
            keyword in text.lower() for keyword in ["静态", "static", "快速", "quick"]
        ):
            hints.append("static")

        if any(
            keyword in text.lower()
            for keyword in ["深度", "deep", "详细", "detailed", "智能"]
        ):
            hints.append("deep")

        if any(
            keyword in text.lower()
            for keyword in ["修复", "fix", "解决", "solve", "优化", "optimize"]
        ):
            hints.append("fix")
...
class ParameterValidator:
    ...
    def validate(self, parsed_input: ParsedInput) -> ValidationResult:
        ...
        try:
            ...
            if parsed_input.input_type == InputType.COMMAND:
                cmd_errors, cmd_warnings, cmd_sanitized = self._validate_command_params(
                    parsed_input
                )
            ...
            # 验证自然语言参数
                ...
                nl_errors, nl_warnings, nl_sanitized = (
                    self._validate_natural_language_params(parsed_input)
                )
            ...
            if "mode" in parsed_input.parameters:
                mode_errors, mode_warnings, mode_sanitized = (
                    self._validate_mode_parameter(parsed_input.parameters["mode"])
                )
            ...
            return ValidationResult(
                is_valid=False, errors=[f"参数验证过程中发生错误: {str(e)}"]
            )

    def _validate_command_params(
        self, parsed_input: ParsedInput
        ...
        if command == "mode":
            ...
            if not format_type or format_type not in [
                "simple",
                "detailed",
                "json",
                "table",
                ...
                errors.append(
                    f"无效的输出格式: {format_type}，支持的格式: simple, detailed, json, table, markdown"
                )
            else:
    ...
    def _validate_natural_language_params(
        self, parsed_input: ParsedInput
    ) -> Tuple[List[str], List[str], Dict[str, Any]]:
        """验证自然语言参数"""
        errors = []
        ...
        if file_paths:
            ...
            for file_path in file_paths:
                ...
                if path_errors:
                    warnings.extend(
                        [f"路径 {file_path}: {error}" for error in path_errors]
                    )
                else:
        ...
        if mode_hints:
            valid_hints = [
                hint for hint in mode_hints if hint in ["static", "deep", "fix"]
            ]
    ...
    def _validate_file_paths(
        ...
        if len(file_paths) > self.validation_rules["max_concurrent_files"]:
            errors.append(
                f"文件数量过多，最多支持 {self.validation_rules['max_concurrent_files']} 个文件"
            )
                ...
                # 检查文件大小
                    ...
                    try:
                        ...
                        if file_size > self.validation_rules["max_file_size"]:
                            warnings.append(
                                f"文件过大: {path_sanitized} ({file_size} bytes)"
                            )
                    except OSError:
    ...
    def _validate_path_security(self, file_path: str) -> Tuple[List[str], str]:
        ...
        if len(file_path) > self.validation_rules["max_path_length"]:
            errors.append(
                f"路径过长 (>{self.validation_rules['max_path_length']} 字符)"
            )
            return errors, ""
...
class ResponseFormatter:
    ...
    def _format_simple_results(
        self, results: List[ExecutionResult], plan: Optional[ExecutionPlan]
    ) -> str:
        """格式化简洁结果"""
        ...
        if failed_results:
            ...
            for i, result in enumerate(failed_results[:5], 1):  # 只显示前5个
                lines.append(
                    f"  {i}. {result.error_message if hasattr(result, 'error_message') else str(result)}"
                )
    ...
    def _format_detailed_results(
        ...
        lines.append("📈 执行统计:")
        lines.append(
            f"  • 成功任务: {success_count}/{total_count} ({success_rate:.1f}%)"
        )
        ...
        for i, result in enumerate(results, 1):
            ...
            lines.append(
                f"{status_emoji} 任务 {i}: {result.task_name if hasattr(result, 'task_name') else 'Unknown'}"
            )
    ...
    def _format_table_results(
        ...
        for i, result in enumerate(results, 1):
            task_name = (
                result.task_name if hasattr(result, "task_name") else f"Task_{i}"
            )[:20]
            ...
            lines.append(
                f"│ {i:^7} │ {task_name:<20} │ {status:<8} │ {exec_time:^12} │"
            )
...
class UserInteractionHandler:
    ...
    def format_response(
        ...
        # 如果是执行结果
        if isinstance(data, list) and data and hasattr(data[0], "success"):
            return self.response_formatter.format_execution_result(
                data, format_type
            ).content
...
⚠️ 中断操作:
  • 按 Ctrl+C 或输入 /stop 中断当前操作
    ...
    def _handle_natural_language(
        self, parsed_input: ParsedInput, validation_result: ValidationResult
    ) -> Dict[str, Any]:
        """处理自然语言输入"""
