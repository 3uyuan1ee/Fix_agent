...
class SecurityMiddleware(AgentMiddleware):
    ...
    def __init__(
        ...
        *,
        ...
        self.sensitive_patterns = {
            ...
            'secret': re.compile(r'(secret|token)\s*[:=]\s*["\']?[a-zA-Z0-9]{20,}["\']?', re.IGNORECASE),
    ...
    def _check_file_security(self, file_path: str, operation: str = "read") -> Optional[SecurityViolation]:
        """检查文件操作安全性"""
    ...
    def _check_command_security(self, command: str) -> Optional[SecurityViolation]:
        ...
        for cmd_name, pattern in self.dangerous_commands.items():
            if pattern.search(command_lower):
                severity = "high" if cmd_name in ['rm', 'dd', 'format', 'shutdown', 'reboot'] else "medium"
                return SecurityViolation(
        ...
        # 2. 管道和重定向检查
        if any(char in command for char in ['|', '&', ';', '`', '$()']) and self.security_level in ['high', 'strict']:
            return SecurityViolation(
    ...
    def _check_content_security(self, content: str) -> List[SecurityViolation]:
        ...
        for info_type, pattern in self.sensitive_patterns.items():
            ...
            if matches:
                severity = "high" if info_type in ['private_key', 'credit_card', 'aws_access_key'] else "medium"

                # 遮蔽敏感信息用于日志
                masked_matches = [match[:4] + "***" + match[-4:] if len(match) > 8 else "***" for match in matches]
    ...
    def _validate_tool_call(self, tool_name: str, tool_args: Dict[str, Any]) -> Optional[SecurityViolation]:
        """验证工具调用安全性"""
                ...
                if '[SECURITY]' in line:
                    ...
                    if len(parts) >= 2:
                        ...
                        if 'path_traversal' in description:
                            violations_by_type['path_traversal'] = violations_by_type.get('path_traversal', 0) + 1
                            ...
                            violations_by_type['sensitive_information'] = violations_by_type.get('sensitive_information', 0) + 1

            return {
                "total_violations": len([line for line in lines if '[SECURITY]' in line]),
