...
try:
    from .multilang_static_analyzer import StaticAnalysisIssue, StaticAnalysisResult
...
class StaticAnalysisAggregator:
    ...
    def aggregate_results(
        self, analysis_results: Dict[str, StaticAnalysisResult], project_path: str
    ...
    def _create_file_summary(
        self, file_path: str, result: StaticAnalysisResult
    ) -> FileAnalysisSummary:
        """
        创建文件分析摘要
        ...
        for position_key, issues_group in position_groups.items():
            if len(issues_group) == 1:
                ...
                aggregated_issues.append(
                    self._convert_to_aggregated_issue(issues_group[0])
                )
            else:
                # 多个问题需要聚合
    ...
    def _convert_to_aggregated_issue(
        self, issue: StaticAnalysisIssue
    ) -> AggregatedIssue:
        """将静态分析问题转换为聚合问题"""
    ...
    def _merge_issues(self, issues: List[StaticAnalysisIssue]) -> AggregatedIssue:
        ...
        # 计算合并后的置信度
        confidence = sum(self._calculate_confidence(issue) for issue in issues) / len(
            issues
        )
    ...
    def _deduplicate_issues(
        self, issues: List[AggregatedIssue]
    ) -> List[AggregatedIssue]:
        """
        全局去重问题
        ...
        for issue in issues:
            ...
            if dedup_key not in seen_keys:
                ...
                for existing_issue in deduplicated:
                    if (
                        existing_issue.file_path == issue.file_path
                        and existing_issue.line_number == issue.line_number
                        and existing_issue.column_number == (issue.column_number or 0)
                        and self._normalize_message_for_dedup(existing_issue.message)
                        == self._normalize_message_for_dedup(issue.message)
                    ):
                        existing_issue.duplicate_count += issue.duplicate_count
                        # 合并工具和规则ID
                        existing_issue.tools = list(
                            set(existing_issue.tools + issue.tools)
                        )
                        existing_issue.rule_ids = list(
                            set(existing_issue.rule_ids + issue.rule_ids)
                        )
                        break
    ...
    def _calculate_complexity_score(
        self, total_issues: int, severity_counts: Dict[str, int], issue_density: float
    ) -> float:
        """计算文件复杂度分数"""
        # 基础分数
    ...
    def _update_language_distribution(
        self, report: ProjectAnalysisReport, language: ProgrammingLanguage
    ) -> None:
        """更新语言分布统计"""
        lang_name = language.value
        report.language_distribution[lang_name] = (
            report.language_distribution.get(lang_name, 0) + 1
        )

    def _update_tools_used(self, report: ProjectAnalysisReport, tool_name: str) -> None:
        """更新工具使用统计"""
        # 这个方法会在分析统计中统一处理
        pass
    ...
    def _calculate_global_statistics(self, report: ProjectAnalysisReport) -> None:
        ...
        # 分析统计
        avg_issues_per_file = (
            report.total_issues / report.total_files if report.total_files > 0 else 0
        )
        high_severity_ratio = (
        ...
        report.analysis_statistics = {
            ...
            "most_used_tool": (
                max(tool_usage.items(), key=lambda x: x[1])[0] if tool_usage else ""
            ),
            "tool_usage": dict(tool_usage),
            "files_with_issues": len(
                [s for s in report.file_summaries if s.total_issues > 0]
            ),
    ...
    def _identify_high_risk_files(
        self, file_summaries: List[FileAnalysisSummary]
    ) -> List[str]:
        """识别高风险文件"""
        ...
        sorted_summaries = sorted(
            file_summaries, key=lambda x: x.importance_score, reverse=True
        )
    ...
    def _normalize_message_for_dedup(self, message: str) -> str:
        ...
        normalized = re.sub(
            r"\b[a-zA-Z_][a-zA-Z0-9_]*\b", "<VAR>", normalized
        )  # 替换变量名
    ...
    def generate_ai_input_data(self, report: ProjectAnalysisReport) -> Dict[str, Any]:
        ...
        file_summaries_for_ai = sorted(
            report.file_summaries, key=lambda x: x.importance_score, reverse=True
        )[
            :50
        ]  # 限制文件数量
        ...
        ai_input = {
            "project_overview": {
                ...
                "analysis_summary": {
                    "avg_issues_per_file": report.analysis_statistics.get(
                        "avg_issues_per_file", 0
                    ),
                    "high_severity_ratio": report.analysis_statistics.get(
                        "high_severity_ratio", 0
                    ),
                    "most_common_category": report.analysis_statistics.get(
                        "most_common_category", ""
                    ),
                },
            },
            ...
            "critical_issues": [
                issue.to_dict()
                for issue in issues_for_ai
                if issue.severity in [SeverityLevel.CRITICAL, SeverityLevel.HIGH]
            ][
                :50
            ],  # 限制严重问题数量
            "sample_issues": [
                issue.to_dict() for issue in issues_for_ai[:20]
            ],  # 示例问题
        }
...
# 便捷函数
def aggregate_static_analysis_results(
    ...
    return {"report": report.to_dict(), "ai_input": ai_input}
