...
class ProblemSkipProcessor:
    ...
    def __init__(self, config_manager=None):
        ...
        self.skip_records_dir = Path(
            self.config.get("skip_records_dir", ".fix_backups/skip_records")
        )
    ...
    def process_skip_decision(
        ...
        try:
            self.logger.info(
                f"处理问题跳过决策: 会话={session_id}, 问题={issue_id}, 原因={skip_reason.value}"
            )
            ...
            self.state_manager.transition_to(
                session_id, WorkflowNode.CHECK_REMAINING, f"问题已跳过: {issue_id}"
            )
            ...
            return {"success": False, "error": str(e), "message": "处理问题跳过失败"}
    ...
    def _remove_problem_from_pending(
        self, session: WorkflowSession, issue_id: str
    ) -> None:
        """从待处理问题列表中移除问题"""
        # 从检测到的问题列表中移除
    ...
    def get_skip_statistics(self, session_id: str) -> SkipStatistics:
        ...
        try:
            ...
            return SkipStatistics(
                total_skipped=0, skip_by_reason={}, skip_by_type={}, skip_by_severity={}
            )
    ...
    def batch_skip_problems(
        ...
        try:
            self.logger.info(
                f"开始批量跳过问题: 会话={session_id}, 数量={len(issue_ids)}"
            )
            ...
            for issue_id in issue_ids:
                try:
                    ...
                    results["results"].append(
                        {"issue_id": issue_id, "success": False, "message": str(e)}
                    )
                    results["failed_count"] += 1

            self.logger.info(
                f"批量跳过完成: 成功={results['success_count']}, 失败={results['failed_count']}"
            )
            return results
