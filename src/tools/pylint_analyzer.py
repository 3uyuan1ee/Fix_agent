...
class PylintAnalyzer:
    ...
    def __init__(self, config_manager=None, config_file: Optional[str] = None, timeout: int = 30):
        ...
        # 默认禁用的代码检查
        self.disable_codes = pylint_config.get('disable_codes', ['C0114', 'C0115', 'C0116'])
    ...
    def analyze_file(self, file_path: Union[str, Path]) -> Dict[str, Any]:
        ...
        try:
            ...
            # 解析结果
            analysis_result = self._parse_pylint_output(result.stdout, result.stderr, result.returncode, file_path)
            ...
            if "pylint" in str(e).lower():
                raise PylintAnalysisError("Pylint is not installed. Please install it with: pip install pylint")
            else:
    ...
    def _parse_pylint_output(self, stdout: str, stderr: str, returncode: int, file_path: Path) -> Dict[str, Any]:
        """解析pylint输出"""
        result = {
            "file_path": str(file_path),
            "score": 0.0,
