...
from .workflow_user_interaction_types import (
    ...
    DecisionContext, DecisionRationale, UserPreference
)
...
class UserDecisionProcessor:
    ...
    def process_single_decision(self, decision_input: DecisionInput,
                               display_data: SuggestionDisplayData) -> DecisionResult:
        """
        处理单个用户决策
    ...
    def process_batch_decisions(self, decision_inputs: List[DecisionInput],
                               display_data_list: List[SuggestionDisplayData]) -> DecisionBatchResult:
        """
        批量处理用户决策
        ...
        try:
            for decision_input, display_data in zip(decision_inputs, display_data_list):
                decision_result = self.process_single_decision(decision_input, display_data)
    ...
    def suggest_decision(self, display_data: SuggestionDisplayData,
                        user_context: Optional[Dict[str, Any]] = None) -> DecisionOutcome:
        """
        基于历史偏好建议决策
        ...
        try:
            ...
            logger.info(f"建议决策: {suggested_outcome.value} (得分: {scores[suggested_outcome]:.2f})")
    ...
    def get_decision_history(self, limit: Optional[int] = None,
                            filter_by_type: Optional[ProblemType] = None) -> List[DecisionResult]:
        """
        获取决策历史
        ...
        for decision in recent_decisions:
            ...
            if problem_type not in type_counts:
                type_counts[problem_type] = {
                    ...
                    "skip": 0, "postpone": 0, "other": 0
                }
            type_counts[problem_type][outcome] = type_counts[problem_type].get(outcome, 0) + 1
    ...
    def _generate_decision_rationale(self, decision_input: DecisionInput,
                                   display_data: SuggestionDisplayData,
                                   outcome: DecisionOutcome) -> DecisionRationale:
        """生成决策理由"""
        primary_factors = []
        secondary_factors = []
        context_factors = []
        ...
        if display_data.code_changes.change_summary:
            secondary_factors.append(f"代码变更: {display_data.code_changes.change_summary}")
        ...
        if decision_input.confidence_level in [DecisionConfidence.UNCERTAIN, DecisionConfidence.VERY_UNCERTAIN]:
        ...
        return DecisionRationale(
            ...
            user_comment=decision_input.user_comment,
            confidence_explanation=self._explain_confidence(decision_input.confidence_level)
        )
    ...
    def _update_user_preferences(self, decision_result: DecisionResult,
                               display_data: SuggestionDisplayData) -> None:
        """更新用户偏好"""
        # 更新质量偏好
    ...
    def _get_relevant_preferences(self, display_data: SuggestionDisplayData) -> Dict[str, Any]:
        """获取相关用户偏好"""
        preferences = {}

        # 质量偏好
        quality_level = self._categorize_quality(display_data.overall_quality_score)
        preferences["quality_preference"] = self._user_preferences.get("quality_preference")
        ...
        # 问题类型偏好
        problem_type = display_data.problem_type
        preferences[f"accept_{problem_type}"] = self._user_preferences.get(f"accept_{problem_type}")

        return preferences

    def _calculate_accept_score(self, display_data: SuggestionDisplayData,
    ...
    def _calculate_reject_score(self, display_data: SuggestionDisplayData,
                              preferences: Dict[str, Any]) -> float:
        """计算拒绝得分"""
        score = 0.0
    ...
    def _calculate_modify_score(self, display_data: SuggestionDisplayData,
                              preferences: Dict[str, Any]) -> float:
        """计算修改得分"""
        score = 0.0
    ...
    def _risk_to_score(self, risk_level: str) -> float:
        """风险等级转得分"""
        risk_scores = {
            "可忽略风险": 1.0, "NEGLIGIBLE": 1.0,
            "低风险": 0.8, "LOW": 0.8,
            "中等风险": 0.5, "MEDIUM": 0.5,
            "高风险": 0.2, "HIGH": 0.2,
            "关键风险": 0.0, "CRITICAL": 0.0
        }
    ...
    def _risk_to_reject_score(self, risk_level: str) -> float:
        """风险等级转拒绝得分"""
        reject_scores = {
            "可忽略风险": 0.0, "NEGLIGIBLE": 0.0,
            "低风险": 0.2, "LOW": 0.2,
            "中等风险": 0.5, "MEDIUM": 0.5,
            "高风险": 0.8, "HIGH": 0.8,
            "关键风险": 1.0, "CRITICAL": 1.0
        }
    ...
    def _calculate_batch_statistics(self, decisions: List[DecisionResult]) -> Dict[str, int]:
        """计算批量统计"""
        stats = {
            "accepted": 0,
            "rejected": 0,
    ...
    def _determine_batch_next_step(self, decisions: List[DecisionResult]) -> WorkflowNode:
        """确定批量处理的下一个工作流步骤"""
        if not decisions:
            return WorkflowNode.USER_REVIEW
