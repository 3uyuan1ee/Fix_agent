...
class ProjectExplorer:
    ...
    def __init__(self):
        self.language_indicators = {
            'python': ['.py', '.pyi', 'Pipfile', 'pyproject.toml', 'requirements.txt', 'setup.py'],
            'javascript': ['.js', '.jsx', '.mjs', '.cjs', 'package.json', 'package-lock.json'],
            'typescript': ['.ts', '.tsx', 'tsconfig.json'],
            'java': ['.java', '.jar', 'pom.xml', 'build.gradle', 'gradlew'],
            'cpp': ['.cpp', '.cc', '.cxx', '.c++', '.c', '.h', '.hpp', '.hxx', 'CMakeLists.txt', 'Makefile'],
            'go': ['.go', 'go.mod', 'go.sum'],
    ...
    def analyze_project(self, project_path: str) -> ProjectAnalysis:
        ...
        try:
            ...
            # 识别架构模式
            architecture = self._identify_architecture_pattern(files, directories, project_type)
            ...
            # 生成建议
            recommendations = self._generate_recommendations(project_type, architecture, metrics)
    ...
    def _categorize_file(self, relative_path: Path, language: str) -> str:
        ...
        # 测试文件
        if (any('test' in part for part in path_parts) or
            filename.startswith('test_') or
            filename.endswith('_test') or
            filename.endswith('.test') or
            'tests/' in str(relative_path)):
            return 'test'
        ...
        # 配置文件
        if (language == 'configuration' or
            filename in ['package.json', 'requirements.txt', 'setup.py', 'pyproject.toml', 'pom.xml', 'build.gradle'] or
            any(folder in path_parts for folder in ['config', 'settings', '.vscode', '.idea'])):
            return 'config'

        # 文档文件
        if (language == 'documentation' or
            filename.endswith(('.md', '.rst', '.txt', '.adoc')) or
            any(folder in path_parts for folder in ['docs', 'doc', 'documentation'])):
            return 'documentation'

        # 构建文件
        if any(folder in path_parts for folder in ['build', 'target', 'dist', 'out']) or \
           filename in ['makefile', 'cmakelists.txt']:
            return 'build'

        # 资源文件
        if any(folder in path_parts for folder in ['assets', 'static', 'resources', 'public']):
            return 'assets'

        return 'other'

    def _identify_project_type(self, files: List[ProjectFile], directories: List[str]) -> ProjectType:
        """识别项目类型"""
        ...
        # Python项目
        if ('setup.py' in file_names or
            'pyproject.toml' in file_names or
            'requirements.txt' in file_names or
            'pipfile' in file_names or
            any('src/' in path for path in file_paths)):
        ...
        if 'package.json' in file_names:
            if any('src/app.js' in path or 'src/app.tsx' in path for path in file_paths):
    ...
    def _identify_architecture_pattern(self, files: List[ProjectFile], directories: List[str], project_type: ProjectType) -> ArchitecturePattern:
        """识别架构模式"""
        dir_names = [d.lower() for d in directories]
        file_names = [f.name.lower() for f in files]

        # 库项目
        if (project_type in [ProjectType.PYTHON_PACKAGE, ProjectType.RUST_CRATE] or
            'setup.py' in file_names or 'cargo.toml' in file_names):
            return ArchitecturePattern.LIBRARY

        # 微服务
        if (any('service' in d for d in dir_names) or
            any('api' in d for d in dir_names) or
            any('microservice' in d for d in dir_names) or
            any('dockerfile' in f for f in file_names)):
            return ArchitecturePattern.MICROSERVICE

        # CLI工具
        if (any('main.py' in f for f in file_names) or
            any('cli' in d for d in dir_names) or
            any('bin' in d for d in dir_names)):
            return ArchitecturePattern.CLI_TOOL

        # API服务
        if (any('api' in d for d in dir_names) or
            any('rest' in d for d in dir_names) or
            any('controller' in d for d in dir_names) or
            any('handler' in d for d in dir_names)):
            return ArchitecturePattern.API_SERVICE

        # Web应用
        if (any('web' in d for d in dir_names) or
            any('frontend' in d for d in dir_names) or
            any('backend' in d for d in dir_names) or
            any('static' in d for d in dir_names) or
            any('templates' in d for d in dir_names)):
    ...
    def _identify_technologies(self, files: List[ProjectFile], directories: List[str]) -> List[Technology]:
        """识别技术栈"""
        technologies = []
        file_contents = {}
        file_names = [f.name.lower() for f in files]
        ...
        for file in files[:50]:  # 限制读取文件数量
            try:
                if file.size < 100000 and file.extension in ['.json', '.py', '.js', '.ts', '.xml', '.yml', '.yaml']:
    ...
    def _analyze_dependencies(self, files: List[ProjectFile], directories: List[str], project_type: ProjectType) -> Dict[str, Any]:
        """分析依赖关系"""
        dependencies = {
        ...
        if project_type == ProjectType.PYTHON_PACKAGE:
            for file in files:
                if file.name in ['requirements.txt', 'pyproject.toml', 'setup.py', 'Pipfile']:
                    try:
                ...
                for line in content.split('\n'):
                    ...
                    if in_dependencies and line:
                        ...
                        deps.append({
                            ...
                            'version': parts[1].strip().strip('"\'') if len(parts) > 1 else 'latest'
                        })
        except Exception:
            pass
        return deps
    ...
    def _calculate_metrics(self, files: List[ProjectFile], directories: List[str], technologies: List[Technology]) -> Dict[str, Any]:
        """计算项目指标"""
    ...
    def _generate_recommendations(self, project_type: ProjectType, architecture: ArchitecturePattern, metrics: Dict[str, Any]) -> List[str]:
        """生成项目建议"""
        recommendations = []
        ...
        # 架构建议
        if architecture == ArchitecturePattern.MONOLITH and metrics['total_files'] > 1000:
...
# 创建工具函数
@tool(name="explore_project_structure", description="深度分析项目结构，识别技术栈、架构模式和依赖关系")
...
@tool(name="analyze_code_complexity", description="分析代码复杂度，识别潜在问题和改进点")
            ...
            try:
                with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
                    ...
                    for i, line in enumerate(lines):
                        ...
                        # 计算圈复杂度指标
                        if any(keyword in line_stripped for keyword in ['if', 'elif', 'for', 'while', 'try', 'except', 'catch', 'switch', 'case']):
                            complexity_score += 1

                        # 统计函数和类
                        if re.match(r'^(def|function|class|interface)\s+\w+', line_stripped):
                    ...
                    # 识别复杂文件
                    avg_complexity = complexity_score / len(lines) if len(lines) > 0 else 0
