...
class ProjectDynamicAnalyzer:
            ...
            if result.returncode == 0:
                for line in result.stdout.splitlines()[1:]:  # 跳过标题行
                    ...
                    if len(parts) >= 11:
                        ...
                        if (project_name in cmd.lower() or
                            project_path_str in cmd or
                            self._is_project_related_process(cmd, project_path)):

                            try:
                                cpu_usage = float(parts[2]) if parts[2].replace('.', '').isdigit() else 0.0
                                memory_kb = float(parts[5]) if parts[5].replace('.', '').isdigit() else 0.0
                            ...
                            processes.append(ProcessInfo(
                                ...
                                memory_mb=memory_mb,
                                parent_pid=int(parts[2]) if len(parts) > 2 and parts[2].isdigit() else 0,
            ...
            if result.returncode == 0:
                ...
                for row in csv_reader:
                    try:
                        ...
                        if cmd and pid > 0:
                            cmd_lower = cmd.lower()
                            if (project_name in cmd_lower or
                                project_path_str in cmd_lower or
                                self._is_project_related_process(cmd, project_path)):
                                ...
                                try:
                                    ...
                                    memory_kb = float(row.get('Mem Usage', '0').replace(' K', ''))
    ...
    def _check_project_specific_processes(self, project_path: Path) -> List[ProcessInfo]:
        """检查项目特定的进程"""
        processes = []

        # 通用项目进程检查
        project_related_patterns = [
            ['python', 'node', 'npm', 'yarn', 'java', 'mvn', 'gradle', 'go', 'cargo', 'docker'],
    ...
    def _is_python_project(self, project_path: Path) -> bool:
        """检查是否为Python项目"""
        python_indicators = ['setup.py', 'requirements.txt', 'pyproject.toml', 'Pipfile']
    ...
    def _check_python_health(self, project_path: Path) -> List[HealthCheck]:
        ...
        venv_exists = any((project_path / indicator).exists() for indicator in venv_indicators)
        ...
        if package_json.exists():
            try:
                ...
                checks.append(HealthCheck(
                    ...
                    response_time_ms=0,
                    error_message=None if node_modules_exists else "Dependencies not installed"
                ))
    ...
    def _measure_performance(self, project_path: Path) -> List[PerformanceMetric]:
        ...
        try:
            if os.name != 'nt':  # Unix-like系统
                ...
                # 内存使用
                mem_result = subprocess.run(['free', '-m'], capture_output_output=True, text=True)
    ...
    def _analyze_python_dependencies(self, project_path: Path) -> Dict[str, Any]:
        ...
        venv_indicators = ['.venv', 'venv', 'env']
        dep_info['has_venv'] = any((project_path / indicator).exists() for indicator in venv_indicators)
                ...
                if dep_file == 'requirements.txt':
                    try:
                        with open(file_path, 'r') as f:
                            ...
                            dep_info['total_dependencies'] = len([d for d in deps if d.strip() and not d.startswith('#')])
                    except Exception:
                        pass
    ...
    def _analyze_node_dependencies(self, project_path: Path) -> Dict[str, Any]:
        ...
        if package_json.exists():
            try:
                ...
                dep_info['total_dependencies'] = len(dependencies) + len(dev_dependencies)
    ...
    def _get_runtime_environment(self, project_path: Path) -> Dict[str, str]:
        ...
        project_env_vars = [
            ...
            'NODE_ENV', 'DJANGO_SETTINGS_MODULE',
    ...
    def _detect_issues(self, project_path: Path, processes: List[ProcessInfo], health_checks: List[HealthCheck]) -> List[Dict[str, Any]]:
        """检测问题"""
        issues = []

        # 检查是否有开发服务器在运行
        dev_server_running = any(
            any(server in proc.command.lower() for server in ['runserver', 'dev', 'start'])
            for proc in processes
        )

        # 检查健康检查结果
        unhealthy_services = [check for check in health_checks if check.status == 'unhealthy']
        warning_services = [check for check in health_checks if check.status == 'warning']
        ...
        if high_memory_processes:
            issues.append({
                ...
                "processes": [{"pid": p.pid, "name": p.name, "memory_mb": p.memory_mb} for p in high_memory_processes]
            })
    ...
    def _generate_recommendations(self, issues: List[Dict[str, Any]], metrics: List[PerformanceMetric]) -> List[str]:
        """生成建议"""
        recommendations = []
        ...
        for issue in issues:
            if issue["type"] == "unhealthy_services":
                recommendations.append(f"发现 {issue['count']} 个不健康的服务，建议检查服务配置")

            elif issue["type"] == "high_memory_usage":
                recommendations.append(f"发现 {issue['count']} 个高内存使用进程，建议优化或重启")
...
# 创建工具函数
@tool(name="analyze_project_dynamics", description="动态分析项目运行状态，包括进程、健康检查、性能指标等")
    ...
    try:
        ...
        result_data = {
            ...
            "summary": {
                ...
                "running_processes": len([p for p in analysis.processes if p.status == "running"]),
...
@tool(name="check_service_health", description="检查项目服务健康状态")
def check_service_health(project_path: str, service_urls: Optional[List[str]] = None) -> str:
    """
    检查服务健康状态
    ...
    try:
        ...
        return json.dumps({
            ...
            "summary": {
                ...
                "healthy_count": len([c for c in health_results if c["status"] == "healthy"]),
                "unhealthy_count": len([c for c in health_results if c["status"] == "unhealthy"]),
                "error_count": len([c for c in health_results if c["status"] in ["error", "timeout", "connection_error"]])
            }
        }, indent=2, ensure_ascii=False)
