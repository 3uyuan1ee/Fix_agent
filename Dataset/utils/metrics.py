...
class MetricsCalculator:
    ...
    def calculate_performance_metrics(
        ...
        return PerformanceMetrics(
            ...
            accuracy=basic_metrics[
                "success_rate"
            ],  # 在这个场景下accuracy = success_rate
            precision=precision,
            recall=recall,
            f1_score=f1_score,
    ...
    def analyze_errors(self, results: List[EvaluationResult]) -> ErrorAnalysis:
        ...
        for result in failed_results:
            ...
            error_analysis["error_types"][error_msg] = (
                error_analysis["error_types"].get(error_msg, 0) + 1
            )
            ...
            error_analysis["error_by_dataset"][dataset][error_msg] = (
                error_analysis["error_by_dataset"][dataset].get(error_msg, 0) + 1
            )

            # 按阶段分析
            phase = self._determine_error_phase(result)
            error_analysis["error_by_phase"][phase] = (
                error_analysis["error_by_phase"].get(phase, 0) + 1
            )

        # 最常见的错误
        error_analysis["most_common_errors"] = sorted(
            error_analysis["error_types"].items(), key=lambda x: x[1], reverse=True
        )[:10]
    ...
    def analyze_time_distribution(
        ...
        for result in results:
            if result.execution_time > 0:
                success_time_data.append(
                    {"success": result.success, "time": result.execution_time}
                )
        ...
        return {
            ...
            "success_time_correlation": {
                ...
                "successful_median": (
                    np.median(successful_times) if successful_times else 0
                ),
    ...
    def compare_datasets(self, results: List[EvaluationResult]) -> Dict[str, Any]:
        ...
        if len(dataset_metrics) > 1:
            best_success_rate = max(
                metrics["success_rate"] for metrics in dataset_metrics.values()
            )
            best_speed = min(
            ...
            for dataset, metrics in dataset_metrics.items():
                metrics["relative_success_rate"] = (
                    metrics["success_rate"] / best_success_rate
                )
                metrics["relative_speed"] = (
                    best_speed / metrics["average_execution_time"]
        ...
        return {
            ...
            "overall_best": {
                "success_rate": (
                    max(dataset_metrics.items(), key=lambda x: x[1]["success_rate"])[0]
                    if dataset_metrics
                    else None
                ),
                "speed": (
    ...
    def calculate_trend_analysis(
        self, results_history: List[List[EvaluationResult]]
    ) -> Dict[str, Any]:
        """
        计算趋势分析
        ...
        return {
            ...
            "improvement": {
                ...
                "success_rate_improvement_pct": (
                    (success_rates[-1] - success_rates[0]) / success_rates[0] * 100
    ...
    def generate_comprehensive_report(
        ...
        results_history: List[List[EvaluationResult]] = None,
    ) -> Dict[str, Any]:
        """
        生成综合评估报告
    ...
    def _generate_recommendations(self, results: List[EvaluationResult]) -> List[str]:
        ...
        if success_rate < 0.5:
            recommendations.append(
                "成功率较低（<50%），建议检查agent的核心逻辑和错误处理机制"
            )
