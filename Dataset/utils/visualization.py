...
class EvaluationVisualizer:
    ...
    def generate_comprehensive_report(self, output_dir: str):
        ...
        self.plot_execution_time_distribution(
            output_path / "execution_time_analysis.png"
        )
    ...
    def plot_success_rate_analysis(self, save_path: str):
        ...
        if "dataset_name" in self.df.columns:
            dataset_success = (
                self.df.groupby("dataset_name")["success"]
                .mean()
        ...
        # 3. 成功率与执行时间的关系
        axes[1, 0].scatter(
            self.df[self.df["success"]]["execution_time"],
    ...
    def plot_execution_time_analysis(self, save_path: str):
        ...
        time_groups = pd.cut(
            self.df["execution_time"], bins=time_bins, labels=time_labels, right=False
        )
    ...
    def plot_error_analysis(self, save_path: str):
        ...
        if "error" in failed_df.columns:
            ...
            if not error_counts.empty:
                ...
                axes[0, 0].set_yticklabels(
                    [e[:30] + "..." if len(e) > 30 else e for e in error_counts.index]
                )
        ...
        if "dataset_name" in failed_df.columns:
            ...
            axes[0, 1].pie(
                dataset_errors.values, labels=dataset_errors.index, autopct="%1.1f%%"
            )
        ...
        if "dataset_name" in failed_df.columns:
            ...
            axes[1, 1].bar(
                x - width / 2, success_rates, width, label="Success Rate", color="green"
            )
            axes[1, 1].bar(
                x + width / 2, error_rates, width, label="Error Rate", color="red"
            )
    ...
    def plot_dataset_comparison(self, save_path: str):
        ...
        bars = axes[0, 1].bar(
            success_rates.index, success_rates.values, color="lightgreen"
        )
        axes[0, 1].set_title("Success Rate by Dataset")
        axes[0, 1].set_ylabel("Success Rate")
        ...
        if len(task_counts) <= 5:  # 只在数据集数量较少时绘制雷达图
            ...
            for dataset in task_counts.index:
                ...
                metrics = [
                    ...
                    1
                    - dataset_data["execution_time"].std()
                    / dataset_data["execution_time"].mean(),  # 一致性
                ]
            ...
            for i, (dataset, metrics) in enumerate(dataset_metrics.items()):
                ...
                axes[1, 1].plot(
                    angles, values, "o-", linewidth=2, label=dataset, color=colors[i]
                )
